...
1. Attack D-085 through D-090 and run every walkthrough command literally.
...
For D-090, the attack will place a dangling decision citation in every plausible source class, including unlisted router Python, nested tests, and nested routing design documents. The guard's claimed scope is broader than its current hand-maintained list of six Python files plus top-level `design/routing/*.md`; the regression must derive the protected scope or narrow the decision explicitly rather than add only the next missed path.

The provenance digest also receives a product-level regression. `IGNORED_DIRS` names `.pytest_cache`, but `managed_source_files()` independently excludes only `__pycache__` and `.git`, so a pytest cache can change installation provenance without appearing in Git status. The fix will exclude `.pytest_cache` from managed provenance, hold that behavior with digest and classification tests, and record the decision. Harness commands will remain cache-safe where they control pytest invocation.
...
M61, M62, and M63 target authority controls in `replay.py` itself. The harness refactor must retarget or re-anchor all three rows so each active target remains unique and still mutates the intended launcher, raw-byte restoration, and anchored-summary control. Those rows are replayed explicitly after retargeting and again as part of the full matrix.
...
Parallel replay can be adopted only after a complete 95-row serial run and a complete 95-row parallel run at the same commit and matrix digest. Both comparison runs are read-only and omit `--write`; the matrix digest is recorded before and after each run and must remain unchanged. The comparison is keyed by row id and requires exact agreement on active or skipped state, caught or survived verdict, skip reason, pytest result counts, and restoration outcome. Missing, extra, duplicated, or reordered report rows are errors. Timing is recorded but is not part of identity.
...
1. Add failing regressions for any reproduced D-085 through D-090 defect, beginning with the policy bypass, driver-name neutralization cases, decision-reference scope, and `.pytest_cache` provenance.
...
3. Implement the smallest guard, provenance, and replay changes that pass those tests.
4. Retarget or re-anchor M61 through M63, prove each target remains unique, and replay those three rows.
5. Run focused tests, the universal validator, the full suite, matrix integrity checks, the canonical serial replay, and then the parallel replay.
6. Run repeated serial and xdist suite trials and compare exact outcome sets.
