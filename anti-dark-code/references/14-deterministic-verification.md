...
The probe reads names, manifests, selected bounded config/code indicators without executing application code. Its profile records evidence paths, scan limits, exclusions, `evidence_classes` and `documentation_only`. It excludes host skill trees, agent worktrees, nested repositories with their own `.git`, mutation-tool sandboxes such as `.stryker-tmp`, and in-tree copies holding a byte-identical root manifest (`scan.skipped_repository_copies`); reviewed `--exclude` entries propagate to planning. Inspect reported dominant unrecognized source extensions, which can make classification incomplete, and `scan.unparsed_manifests`: a manifest the probe could not parse has unknown scripts, not none, and the text output warns for each.
