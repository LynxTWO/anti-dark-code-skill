...
- Keep verification artifacts that need exclusive file access out of trees that developer tooling indexes. Editors, language servers, indexers, and sync clients hold transient handles, and a cleanup or rename step then fails intermittently with timeouts that read as flaky gates. Put scratch and gate artifacts in a dedicated untracked directory the tooling ignores. Where a handle-enumeration mechanism exists, use it to name the holder before proposing any timing remedy; where none exists, treat absence of a user-mode holder as evidence pointing at a kernel filter rather than as contention ruled out. Identify the holder first: a retry or a longer timeout added before the diagnosis hides the contention instead of removing it, and a single-file survivor pattern points at an indexer rather than at leaked child processes. Where the remedy is a tool exclusion, record it beside the gate as an environmental prerequisite. It is a property of the developer environment that the repository cannot enforce from the inside, so a result that depends on it is not reproducible without it.
...
- A producer that is not a gate declares an output root outside any audited or lease-protected tree, and the reason is commented at the path definition so the next author does not move it back. Writing into an audited tree is an opt-in to that tree's obligations, lease, invalidation, atomic publish, and audit, whether or not the author noticed. In one repository a timing harness written beside the gate evidence placed an unaudited file inside a tree certified as a set on its first successful run; the path moved to a sibling measurements root before the harness was committed.

### 9c. A suite's verdict is defined by its configured runner
...
- Record the canonical invocation beside the suite, and use it in evidence. A pass or failure count without the command that produced it is not reproducible evidence.
- When an ad-hoc run disagrees with the canonical runner, distrust the ad-hoc run first. The burden of proof sits on the unusual invocation, not on the code.
- A run that discovers zero tests is a failed invocation, never a pass and never evidence of absence. Discovery reporting nothing means the runner looked in the wrong place; treat its exit as noise until the canonical command reproduces it.
- When a spurious failure has been observed and explained, record the ghost and its explanation where the next reader of the evidence will look. In one dogfeeding incident, an ad-hoc invocation from the wrong root produced four plausible failures on a suite that passes eight of eight under its package runner; the note recording that ghost is what stops the next reviewer from re-debugging an invocation artifact as a product defect.
...
- Every required continuous-integration job keeps a measured margin between its normal duration and its timeout, at least two to one, or it is split. A job that runs at ninety-six percent of its ceiling passes alone and fails under shared-runner contention, and the failure reads as a defect in the change under review. Re-measure before opening several pull requests at once. In one repository a 25-minute replay job that had always finished in 24 to 25 minutes was canceled at 25 minutes 15 seconds when three pull requests ran it together; the same tree passed on another of the three.
