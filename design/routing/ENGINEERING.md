...
| R-053 | data-driven replay of all 106 stored mutations | `mutants/`; 101 active and 5 superseded under D-094. Round Eighteen's full clean T540P serial `--write` replay at `c846660` processed all 106 rows with 0 not caught, restored every active source, and supplied Linux records for M97 through M106. Every active row now carries Windows and Linux records. M37, M46, and M48 are `caught elsewhere` only because the exact Windows skipped node is the exact Linux failed node (D-104). An unrelated or unattributed skip cannot soften a survivor, and a zero-skip survivor retains D-095 precedence. Source restoration is hash-verified by D-068 |
