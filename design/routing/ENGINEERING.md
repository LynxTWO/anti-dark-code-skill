...
| R-053 | data-driven replay of all 151 stored mutations | `mutants/`; 145 active and 6 superseded under D-094 and D-113. SLICE-002's two full serial `--write` replays at `6449d1f`, one on Windows and one on WSL2 Ubuntu, run concurrently from independent clean clones, each processed all 151 rows with 0 not caught; their records were merged per platform by `derive_verdict` (D-109's principle, applied without a sequential same-file write this time since the two runs shared no matrix.json), so every active row carries exact failed and skipped identities from both hosts at one commit. M127 to M138 were added across the SLICE-002 implementation and M139 to M151 across its revision and repair, all twenty-five carrying no host record until this replay; every earlier row down to M01 was refreshed too, so all 145 active rows date to this one commit rather than a mix of this round's and round twenty-one's. M37, M46, and M48 are `caught elsewhere` only because the exact Windows skipped node is the exact Linux failed node (D-104). The sanity pass that preceded the two writes found and D-132 closed a CRLF anchoring gap in three source files the matrix targets, affecting a casual Windows checkout only, never either replay clone. Source restoration is hash-verified by D-068 |
