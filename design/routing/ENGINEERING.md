...
| R-053 | data-driven replay of all 109 stored mutations | `mutants/`; 104 active and 5 superseded under D-094. Round Nineteen's full Windows serial `--write` replay at `39d745d` processed all 109 rows with 0 not caught from a clean clone and refreshed every Windows record with exact failed and skipped identities (D-109); a read-only parallel replay at the same head agrees with it row for row. The Linux records are Round Eighteen's T540P records at `c846660`; M107, M108, and M109 await theirs, and the Linux CI job has caught all three on every run since `39d745d`. M37, M46, and M48 are `caught elsewhere` only because the exact Windows skipped node is the exact Linux failed node (D-104). Source restoration is hash-verified by D-068 |
