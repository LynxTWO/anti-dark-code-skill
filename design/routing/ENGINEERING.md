...
| R-053 | data-driven replay of all 114 stored mutations | `mutants/`; 108 active and 6 superseded under D-094 and D-113. Round Twenty's two full serial `--write` replays at `0ace58f`, one on Windows and one on WSL2 Ubuntu, each processed all 114 rows with 0 not caught from a clean clone; their records were merged per platform, so every active row carries exact failed and skipped identities from both hosts at one commit (D-109); a read-only Windows parallel replay at the same head agrees with the Windows write row for row. The Linux records are Round Eighteen's T540P records at `c846660`; M107, M108, and M109 await theirs, and the Linux CI job has caught all three on every run since `39d745d`. M37, M46, and M48 are `caught elsewhere` only because the exact Windows skipped node is the exact Linux failed node (D-104). Source restoration is hash-verified by D-068 |
...
The path table has one test per class. A newly authoritative config that is not represented makes the table test fail closed until reviewed. D-097 tried to infer script authority from an `adc*` name and a quoted-loader scan. A dynamically assembled `importlib` load bypassed that scan. D-100 replaces the inference: every Python file directly under `anti-dark-code/scripts/` is verification authority by location, and `test_every_shipped_script_is_authority_by_location` holds the rule.
