...
Date: 2026-09-04. From: Claude Fable 5.1, which decided D-133 to D-135 with the owner and stopped. For: Opus 5, which implements and stops. After you: Sonnet 5 runs the periodic ingest and summary, here and for case-b; the top tier returns only for the approval review.

## Why there is a third handoff

The implementation challenge left two design debts open and named them: ingest recomputes the verdict but not the class, and the suite reads a prose file so the `docs-only` canary exists only because a test asserts that file exists. The original source-specific consent is withheld and grants no permission for the illustrative example. The selected historical case met criterion 4, and the shipped skill carries `adc_shadow.py` with nothing explaining it. D-133, D-134 and D-135 decide all four, and brief version 0.5 carries them as M8 to M11.
...
5. `design/routing/consumers/example-consumer/README.md`, an illustrative historical configuration shape; it carries no approval for any repository.
...
| M10 | The consumer kit ships: `references/shadow-evidence.md` (written; keep it true), `assets/templates/shadow-job.yml` and `assets/templates/shadow-gate-map.json` (written), the SKILL.md line under supporting references (written), and the CHANGELOG entry naming all four (written). Ingest and summary accept a consumer ledger directory as `--ledger` and keep `policies/` beside it; a test ingests a fixture consumer clone into `metrics/shadow/consumers/<slug>/` and hashes the clone before and after. Then the release: VERSION to the next number, the CHANGELOG `Unreleased` section moved under it, the tag, `release-check` against the tag. The installer refuses an untagged source, so case-b cannot upgrade before this. | S-069; R-067; `validate --mode distribution` clean; `release-check` clean; D-135 |
| M11 | With the historical consumer owner: case-b's historical pull-request plan, subject to its own owners' review. `consumers/example-consumer/README.md` now describes only an unbound illustration, not that source-specific plan. Then its first live record ingested under `metrics/shadow/consumers/example-consumer/`, and the backfill over its 45 pull requests' run history into the same ledger. | the first consumer record; D-135 |
| Report back | `HANDOFF-BACK-SLICE-002-IMPL-R3.md`: what the re-ingest refused and why, the dominance result for `docs-only` here, the release tag, and case-b's first record and class mix. | the brief's section 11 |
...
- Do not write anything to case-b beyond the four files and the job, and do not copy this repository's calibration into it.
...
Stop and report after M10's release is tagged and checked, before M11's pull request is opened, so the owner and the historical consumer owner open it. Stop earlier if M8's re-ingest refuses a record it cannot recover, if a dominance probe cannot restore the tree, or before any write to a repository that is not this one.
