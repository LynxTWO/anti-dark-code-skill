...
- [x] Platform execution is split from acceptance coverage. PR 29 commit `39d745d5720ef629231a4c17563be818399141f5` passed Linux, macOS, and Windows, plus Python 3.13, in required run `33656382905` on its second attempt; that run also passed both hostile-environment jobs, the clean distribution check, the complete Linux mutation replay of all 109 rows, and the aggregate gate. Its first attempt failed only the macOS suite leg on a temporary-directory cleanup race in `test_efficiency.py`, recorded as an open unknown. The run before it, `33654922514` at `49fed51`, failed its Linux mutation replay on M107, which was the round-nineteen test grading itself wrongly and being caught. Round Eighteen's equivalent receipt was run `33645108730` at `08d0576`; Round Eighteen also ran the full 106-row serial matrix on T540P and challenged R-032 through the real classifier on Windows and Linux. The owner walkthrough still requires the final branch checks on its own head.
...
- [x] Every stored mutation is replayable from data. The matrix has 109 rows, 104 active and 5 superseded. Round Nineteen's full Windows serial `--write` replay at `39d745d` processed all 109 rows with `0 not caught` from a clean clone and refreshed every Windows record with exact identities (D-109); a read-only parallel replay at the same head agrees with it row for row, and the Linux CI replay in run `33656382905` caught all 109. The Linux records are Round Eighteen's T540P records at `c846660`; M107, M108, and M109 await their per-row Linux record. M37, M46, and M48 are `caught elsewhere` because the exact Windows skipped node is the exact Linux failed node (D-104). The harness restored every active mutation source to its exact pre-run hash (D-068). `SERIAL-EVIDENCE-ROUND-NINETEEN.json` records the three Windows runs, the record-currency measurement, and the boundaries; `SERIAL-EVIDENCE-ROUND-EIGHTEEN.json` remains the Linux record.
