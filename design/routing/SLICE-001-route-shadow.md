...
- [x] Platform execution is split from acceptance coverage. PR 32 commit `5872e922fc6ff5bf4f7f3e879ffc50fb194871c1` passed Linux, macOS, and Windows, plus Python 3.13, in required run `33699688243` on its first attempt; that run also passed both hostile-environment jobs, the clean distribution check, the complete Linux mutation replay of all 118 rows, and the aggregate gate. The run before it, `33697471094` at `6930274`, was also green on its first attempt; the difference between the heads is D-119 and the source-half contract test, both from a fresh-context challenge. Round Twenty's equivalent receipt was run `33671714111` at `0ace58f`; Round Nineteen's was `33656382905` at `39d745d` on its second attempt after a temporary-directory cleanup race recorded as an open unknown. Round Eighteen's was run `33645108730` at `08d0576`; Round Eighteen also ran the full 106-row serial matrix on T540P and challenged R-032 through the real classifier on Windows and Linux. The owner walkthrough of 2026-09-02 approved the slice at `3e04422`.
...
- [x] Every stored mutation is replayable from data. The matrix has 118 rows, 112 active and 6 superseded. Round Twenty-One's two full serial `--write` replays at `5872e92`, one on Windows and one on WSL2 Ubuntu, each processed all 118 rows with `0 not caught` from a clean clone; their records were merged per platform, so every active row carries exact failed and skipped identities from both hosts at one commit (D-109), and a read-only Windows parallel replay at the same head agrees with the Windows write row for row. The Linux CI replay in run `33699688243` caught all 118. M100 was re-anchored and M115 and M116 added under D-118, M117 and M118 under D-119, and all five carry records from both hosts. M37, M46, and M48 are `caught elsewhere` because the exact Windows skipped node is the exact Linux failed node (D-104). M08 is superseded by M114 because its catch on three hosts was each host's git-lfs driver (D-113). The harness restored every active mutation source to its exact pre-run hash (D-068). `SERIAL-EVIDENCE-ROUND-TWENTY-ONE.json` records the two writes, the parallel replay, the measurements at `6930274`, the merge, and the boundaries.
