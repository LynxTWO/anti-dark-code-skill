...
- [x] Platform execution is split from acceptance coverage. PR 27 commit `4b24122a6051109461d4d82826af34a6a84fca68` passed Linux, macOS, and Windows, plus Python 3.13, in required run `33610038786`; that run also passed both hostile-environment jobs, the clean distribution check, the complete Linux mutation replay of all 99 rows, and the aggregate gate. Round Sixteen's equivalent receipt was run `33590598057` at `429f0d7`. The documentation commits after `4b24122` change no mutation source; their exact-head receipt is recorded in `HANDOFF-BACK-ROUND-SEVENTEEN.md` once observed.
...
- [~] D-080 withdraws the unreconstructible claim that EDD section 17 passed for every historical change. Before `ea8733c`, the replacement is the qualified slice-level evidence here. From `ea8733c` forward, every commit carries `EDD-Checklist: satisfied` and names the host or workflow evidence used, except the seven round-fifteen commits `30c577c` through `bf9aba3`, which carry no trailer. D-080 says such commits violate it; round seventeen measured the range (49 commits, 7 without the trailer) and records the violation rather than rewriting history. Those seven are covered by `HANDOFF-BACK-ROUND-FIFTEEN.md` and required run `33434352766` at `9dd7a3b`.
...
- [x] Every stored mutation is replayable from data. The matrix has 99 rows, 94 active and 5 superseded. Round Sixteen's frozen full T540P Linux and Windows replays processed the first 96 rows with `0 not caught`, and all 91 of those active rows carry both host records. Round Seventeen added M97, M98, and M99 (D-095, D-096, D-098): the full Windows parallel replay at `4b24122` processed all 99 rows with `0 not caught`, the serial oracle run of the three new rows agrees with it row for row and supplies their Windows records, and the Linux CI replay in run `33610038786` caught all three; their per-row T540P record is pending. M37, M46, and M48 are `caught elsewhere`, held by a symlink test Windows skips, and the replay summary now discloses them. The harness restored every mutation source to its exact pre-run hash (D-068); `PARALLEL-EVIDENCE-ROUND-SIXTEEN.json` and `PARALLEL-EVIDENCE-ROUND-SEVENTEEN.json` record the frozen commits, matrix digests, per-row outcomes, and restoration evidence.
...
- [x] Documents record the actual Task 13 command, receipt and refusal results, the 30-item closure ledger, the evidence qualifications, D-080 through D-084, and the owner walkthrough.
