...
- [x] Platform execution is split from acceptance coverage. PR 27 commit `4b24122a6051109461d4d82826af34a6a84fca68` passed Linux, macOS, and Windows, plus Python 3.13, in required run `33610038786`; that run also passed both hostile-environment jobs, the clean distribution check, the then-current Linux mutation replay of all 99 rows, and the aggregate gate. Round Sixteen's equivalent receipt was run `33590598057` at `429f0d7`. Round Eighteen separately ran the full 106-row serial matrix on T540P and challenged R-032 through the real classifier on Windows and Linux. The owner walkthrough still requires the final Round Eighteen branch checks on its own head.
...
- [x] Every stored mutation is replayable from data. The matrix has 106 rows, 101 active and 5 superseded. Round Eighteen refreshed the skip-sensitive Windows rows and M100 through M106 from a clean `core.autocrlf=false` clone, then ran the full serial `--write` matrix on T540P from a second clean clone at `c846660`. The Linux run processed all 106 rows with `0 not caught`; every active row now carries both host records, including M97, M98, and M99. M37, M46, and M48 are `caught elsewhere` because the exact Windows skipped node is the exact Linux failed node. D-104 refuses that label for an unrelated or missing identity. The harness restored every active mutation source to its exact pre-run hash (D-068). `SERIAL-EVIDENCE-ROUND-EIGHTEEN.json` records the commits, report and matrix digests, endpoint statuses, and per-round boundaries.
