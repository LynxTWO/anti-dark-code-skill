...
- [x] Platform execution is split from acceptance coverage. PR 26 commit `429f0d7da0838c5f6f73143587f4cd5cc8f272d4` passed Linux, macOS, and Windows, plus Python 3.13, in required run `33590598057`; that run also passed both hostile-environment jobs, the clean distribution check, the complete Linux mutation replay, and the aggregate gate. This documentation-only CI receipt follows that proven implementation and review head.
...
- [~] D-080 withdraws the unreconstructible claim that EDD section 17 passed for every historical change. Before `ea8733c`, the replacement is the qualified slice-level evidence here. From `ea8733c` forward, every commit carries `EDD-Checklist: satisfied` and names the host or workflow evidence used.
...
- [x] Every stored mutation is replayable from data. The final matrix has 96 rows, 91 active and 5 superseded under D-094. Frozen full T540P Linux and Windows replays each processed all 96 rows with `0 not caught`; all 91 active rows carry both host records. M37, M46, and M48 are `caught elsewhere`, held by a symlink test Windows skips. The harness restored every mutation source to its exact pre-run hash (D-068), and `PARALLEL-EVIDENCE-ROUND-SIXTEEN.json` records the frozen commits, matrix digest, per-row outcomes, and restoration evidence.
...
- [~] Current and forward EDD guardrails hold with no unlabeled shortcut inside the boundary. D-080 withdraws the unsupported historical per-change claim and anchors the forward review record at `ea8733c`.
