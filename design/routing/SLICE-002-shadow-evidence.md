...
- **A consumer repository.** A repository that installs the campaign carries the skill at a version shipping `adc_shadow.py`, its own calibration with every rule `proposed`, its own `.github/shadow-gate-map.json`, and the `shadow` job from `assets/templates/shadow-job.yml` with `needs` naming its gate jobs. Its evidence lives here, under `metrics/shadow/consumers/<owner>-<name>/` in this same shape, ingested from a clone of the consumer with `--repo <clone> --repository <owner>/<name>` and `--ledger` naming that directory. Nothing under `metrics/` is written to the consumer, and its calibration is authored for it, never transplanted (D-135). The first consumer is case-b, section 12.
...
**The second path: dominance.** A class whose covered paths no omitted gate reads cannot produce a canary, and its clean records prove only that nothing happened. Such a class is approved instead by a dominance proof (D-134): `adc.py shadow dominance --class-key <key>` enumerates every path in the tree the class's classifier entries cover, runs the full recipe with every one of them deleted, then again with every one of them replaced by content that is not the original, and records every gate's conclusion each time. The class is dominated when neither probe produces a selected gate passing while an omitted gate fails; a dominated class needs neither criterion 1 nor criterion 3, because the proof is the comparator seeing every failure the class can cause and finding each caught by a selected gate or caused by nothing. A probe that produces a miss makes the class not dominated, and that probe's record is then the class's canary for the statistical path. The proof is an approval-time act, re-run whenever the class key changes, and its records carry `provenance: dominance`, listed by the summary beside canaries and counted in neither N nor the misses. This repository's `docs-only` class cannot take this path either: its gates carry no local commands, so there is nothing for the probe to run, and D-137 records the class as approvable by neither path rather than engineering around it. The probe ships and is held by tests, so a consumer whose gates are commands can use it on the first day; making it a CI act that dispatches the real jobs is the owner's chosen second step and belongs to SLICE-003.
...
| M11 | case-b's pull request, with its owners: the skill upgraded, its original source-specific proposal reviewed for its own calibration and map (the directory `design/routing/consumers/example-consumer/` now retains only an unbound illustration), the job added with `needs: [test]`, its required check untouched; the first live record ingested under `metrics/shadow/consumers/example-consumer/` | the first consumer record; D-135 |
...
- [x] The suite reads no repository prose (D-134). The `docs-only` class here has no dominance record and will not get one: this repository's gates carry no local commands, so the probe cannot run here, and the class is approvable by neither path (D-137). The first repository to exercise either is a consumer whose gates are commands.
- [ ] The shipped skill documents the campaign and carries the consumer templates (D-135).
- [ ] case-b's first live record is ingested under `metrics/shadow/consumers/example-consumer/`.
...
Before SLICE-003, the campaign's first consumer. This repository has one author and cannot meet criterion 4; case-b has two, 45 pull requests, and a history in which every record was measurable, and its source-specific installation permission is withheld; no permission transfers to the example. It is the first campaign that can reach N, and the first place the audit product D-130 names is measured rather than argued (D-135). Its time prize is nothing, a 34-second CI, and that is the point: the graded record is the product.
...
6. **The backfill grades pull-request run history, superseded attempts included.** A merge commit is survivorship: on case-c, lint failed on none of 216 push runs for merges and in 57 of 309 pull-request runs, one of them a routed miss. The merge-commit replay is retired; D-128.
...
13. **A consumer carries the job, the map and its calibration; its evidence lives here.** case-b is the first consumer, with source-specific consent withheld and no permission transferred; nothing under `metrics/` is written to it and nothing of this calibration is transplanted; D-135.
...
15. **This repository's `docs-only` class stays approvable by neither path, and dominance becomes a CI act later.** Its gates carry no local commands, so the probe cannot run here; authoring commands would flip a posture D-011 and the calibration state deliberately, to prove a class in the one repository whose history can never meet the second-author condition anyway. Taken 2026-09-04 as "three now, two later"; D-137.
