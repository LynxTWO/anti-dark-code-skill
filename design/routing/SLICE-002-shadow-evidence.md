...
- [ ] The owner has chosen N, A, and D, the storage option, and whether to narrow `docs-only` before counting.
...
1. N, A, and D from the table in section 6; option 1 is the recommended first milestone.
2. Storage option A, artifacts ingested into a committed ledger with recomputation, or option B, a ledger branch written by the workflow.
3. Whether to narrow the `docs-only` classifier so that prose the suite reads is not `docs`, before the campaign counts anything; without it the first canary is a real miss and the class cannot meet the criterion.
4. Whether backfilled records may ever count toward N, or only inform.
5. Whether a canary is run once per class key or once per policy change.
